...
public class Element
{
	...
	private static final int[] SUB_SHELL_ELECTRONS = new int[]{2,6,10,14,18};
	...
	/**
	 * Iterates over the {@code SHELL_ORDER} string, calculates the size of the sub-shell from it's type
	 * and subtracts it from the electron count. when that is count <= 0 the latest sub-shell is the valence shell
	 * and full shell electrons + the count (which is normally -ve) gives the valence electrons.
	 * Can easily be modified to recover the Shell no. separately to the full "1s" value by changing the SHELL_ORDER pattern to:
	 * (//d+)([a-z]) and the casting group(1) to an int, and using group(2) to find the sub-shell type
	 */
